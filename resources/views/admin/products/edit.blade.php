                    ...
                    <form method="POST" action="{{ route('admin.products.update', $product->id) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" rows="3" required
                                      class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $product->description) }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="number" name="price" value="{{ old('price', $product->price) }}" step="0.01" min="0.01" required
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                            </div>
                            <div>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="image_url" value="{{ old('image_url', $product->image_url) }}" placeholder="images/products/example.jpg"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                        </div>

                        {{-- Sizes Section --}}
                        <div class="mb-6 border-t pt-4">
                            <h3 class="text-lg font-medium text-gray-900 mb-3">Sizes & Stock</h3>
                            <div id="sizes-container">
                                @foreach($product->sizes as $i => $size)
                                    <div class="flex items-center gap-3 mb-2">
                                        <input type="hidden" name="sizes[{{ $i }}][id]" value="{{ $size->id }}">
                                        <input type="text" name="sizes[{{ $i }}][size]" value="{{ $size->size }}" placeholder="e.g. S, M, L"
                                               class="flex-1 px-3 py-2 border border-gray-300 rounded-md text-sm">
                                        <input type="number" name="sizes[{{ $i }}][stock_quantity]" value="{{ $size->stock_quantity }}" min="0" placeholder="Stock"
